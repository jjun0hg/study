# 다중분류 // sparse_categorical//iris data 
...
x_train, x_test, y_train, y_test = train_test_split(
    ...
    #random_state=333, 
    test_size=0.2,
    stratify=y                      # 분류형 데이터일 경우 가능하다(분류에서만 사용)
)
# print(y_train)
# print(y_test)
...
#3. 컴파일, 훈련
model.compile(loss='sparse_categorical_crossentropy', optimizer='adam',    # sparse_categorical_crossentropy 로 변경해도 가능
              metrics=['accuracy'])
...
# y_test = np.argmax(y_test, axis=1)                          # np.argmax, acc 필요 x
