...
x_train = x_train.reshape(1062, 3, 3)
x_test = x_test.reshape(266, 3,3)

# #2. 모델구성
model = Sequential()
model.add(LSTM(units=350, input_shape=(3,3), 
...
mcp = ModelCheckpoint(monitor='val_loss', mode = 'auto', verbose = 1,
                      save_best_only=True,
                      filepath = filepath + 'k48_04_' + date +'_'+ filename)                   
...
# 제출
y_submit = model.predict(test_csv.reshape(715, 3,3))
submission['count'] = y_submit  # y_submit 저장
submission.to_csv(path +"submission_0126.csv")
